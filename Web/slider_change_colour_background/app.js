...
redSlider.oninput = function () {
    ...
    body.style.backgroundColor = `rgb(${red_val}, ${green_val}, ${blue_val}, ${alpha_val}%)`;
};
...
alphaSlider.oninput = function () {
    alpha_val = this.value;
    const min = this.min ? this.min : 0;
    const max = this.max ? this.max : 0;
    const offset = Number((alpha_val - min) * 100 / (max - min));
    //Move the indicator
    info[3].innerText = alpha_val;
    let parent = info[3].parentElement;
    parent.style.left = `calc(${offset}% + ${1 - offset*.26}px)`;
    body.style.backgroundColor = `rgb(${red_val}, ${green_val}, ${blue_val}, ${alpha_val}%)`;
};
