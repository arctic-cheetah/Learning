...
form.addEventListener("submit", function(e) {
  ...
  //Note: decimals are disallowed by the form

  //Consider cases from user input
  //Negative, empty input
  //Alert the user to enter positive integers
  if (isNaN(value) || value <= 0) {
    ...
    let str = "";
    let arr_len = text.length;
    //Generate random number between 0 - 8
    for (let i = value; i > 0; i -=1) {
      let num = Math.round(Math.random() * (arr_len - 1));
      str += (text[num] + "<br><br>");
    }
